...
public class Cache {
  ...
  private static class CacheMap<K, V> extends LinkedHashMap<K, V> {
    ...
    void setMaxSize(int maxsize) {
      /*
       * Note that this doesn't shrink the size of the map if the maximum size is lowered, but it
       * should shrink as entries expire.
       */
      this.maxsize = maxsize;
    }
  ...
  private CacheMap<Name, Object> data;
  ...
  protected synchronized SetResponse lookup(Name name, int type, int minCred) {
    ...
    for (tlabels = labels; tlabels >= 1; tlabels--) {
      ...
      /*
       * If this is the name, look for the actual type or a CNAME (unless it's an ANY query, where
       * we return everything). Otherwise, look for a DNAME.
       */
   ...
   * specific Name. A negative value is treated as an infinite limit.
   *
   * Note that setting this to a value lower than the current number of entries will not cause the
   * Cache to shrink immediately.
   *
