...
final class UDPClient extends Client {
   ...
   * local SecureRandom object, the thread hangs.
   * 
   * To avoid both of these, check at initialization time to see if InetAddress is in the call
   * chain. If so, initialize the SecureRandom object in a new thread, and disable port
   * randomization until it completes.
   */
  static {
  ...
  static byte[] sendrecv(SocketAddress local, SocketAddress remote, byte[] data, int max, long endTime)
      throws IOException {
