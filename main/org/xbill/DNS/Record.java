...
public abstract class Record implements Cloneable, Comparable<Record>, Serializable {
   ...
   *         than 0 if the argument is greater than this record in the canonical ordering. The
   *         canonical ordering is defined to compare by name, class, type, and rdata.
  ...
  static byte[] checkByteArrayLength(String field, byte[] array, int maxLength) {
    if (array.length > 0xFFFF)
      throw new IllegalArgumentException("\"" + field + "\" array " + "must have no more than " + maxLength
          + " elements");
