 ...
 *
 * The option is used to convey information about the IP address of the originating client, so that
 * an authoritative server can make decisions based on this address, rather than the address of the
 * intermediate caching name server.
 *
 * The option is transmitted as part of an OPTRecord in the additional section of a DNS message, as
 * defined by RFC 2671 (EDNS0).
 *
 * The wire format of the option contains a 2-byte length field (1 for IPv4, 2 for IPv6), a 1-byte
 * source netmask, a 1-byte scope netmask, and an address truncated to the source netmask length
 * (where the final octet is padded with bits set to 0)
 *
 *
 * @see OPTRecord
 *
