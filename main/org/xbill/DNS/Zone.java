...
public class Zone implements Serializable {
    ...
    public Object next() {
      ...
      if (count == current.length) {
        ...
        while (zentries.hasNext()) {
          ...
          Map.Entry<Name, Object> entry = (Map.Entry<Name, Object>) zentries.next();
  ...
  private void fromXFR(ZoneTransferIn xfrin) throws IOException, ZoneTransferException {
    ...
    @SuppressWarnings("unchecked")
    List<Record> records = (List<Record>) xfrin.run();
  ...
  private synchronized SetResponse lookup(Name name, int type) {
    ...
    for (tlabels = olabels; tlabels <= labels; tlabels++) {
      ...
      /*
       * If this is the name, look for the actual type or a CNAME. Otherwise, look for a DNAME.
       */
      if (isExact) {
  ...
  public synchronized String toMasterFile() {
    Iterator<Map.Entry<Name, Object>> zentries = data.entrySet().iterator();
